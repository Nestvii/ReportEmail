...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
    ...
    print(f"{C['bold']}{C['white']}{' ' * 23}ᯤ Version {C['reset']} 2.0")
...
def add_template():
    ...
    # Email Tujuan
    while True:
        target = input("  Email Tujuan : ").strip()
        if target.upper() == "X":
            return
        if validate_email(target):
            break
        print(f"{C['red']}  ✗ Format email tidak valid.{C['reset']}")
    ...
    print(f"  {C['darkgray']}(Ketik selesai pada baris kosong untuk mengakhiri){C['reset']}\n")
    text_lines = []
    while True:
        line = input("  > ")
        if line.strip().upper() == "X":
            return
        if line.strip().lower() == "selesai" and not text_lines:
            # Allow empty if user explicitly says selesai with no content
            break
        if line == "" and text_lines:
            break
        if line == "" and not text_lines:
            print(f"  {C['yellow']}◈ Teks kosong. Ketik selesai untuk mengakhiri, atau isi teks.{C['reset']}")
            continue
        text_lines.append(line)
    text = "\n".join(text_lines) if text_lines else "Laporan otomatis."
...
def edit_template():
    ...
    while True:
        ...
        if edit_choice == "1":
            ...
            while True:
                val = input("  Email Tujuan baru : ").strip()
                if val.upper() == "X":
                    break
            ...
            while True:
                ...
                if line == "" and text_lines:
                    break
                if line == "" and not text_lines:
                    print(f"  {C['yellow']}◈ Teks kosong. Ketik selesai untuk mengakhiri.{C['reset']}")
                    continue
                text_lines.append(line)
            if not cancelled:
                edited['text'] = "\n".join(text_lines) if text_lines else edited.get('text', '')
...
def start_report():
    ...
    if use_template and selected_template:
        ...
        while True:
            tujuan = input("  Email tujuan : ").strip()
            if tujuan.upper() == "X":
                return
            if validate_email(tujuan):
                break
            print(f"{C['red']}  ✗ Format email tidak valid.{C['reset']}")
        ...
        print(f"  {C['darkgray']}(Ketik selesai pada baris kosong){C['reset']}\n")
        text_lines = []
        while True:
            line = input("  > ")
            if line.strip().upper() == "X":
    ...
    try:
        for i in range(1, jumlah + 1):
            ...
            if status:
                ...
                sys.stdout.write(f"\r  {C['green']}✓ [{i}/{jumlah}] Berhasil{C['reset']}  ")
            else:
                total_gagal += 1
                log_result(f"FAILED  | #{i} | {sender['email']} → {tujuan}")
                sys.stdout.write(f"\r  {C['red']}✗ [{i}/{jumlah}] Gagal{C['reset']}  ")
            sys.stdout.flush()
            if i < jumlah:
